...
- (NSString *)jk_commentedString {
  NSRegularExpression *expression =
      [[NSRegularExpression alloc] initWithPattern:@"^\\s*" options:0 error:NULL];
  NSRange range =
      [expression rangeOfFirstMatchInString:self options:0 range:NSMakeRange(0, self.length)];
...
- (NSString *)jk_commentRemovedString {
  ...
  if ([trimmedString jk_isAcomment]) {
    ...
    trailingSpaces = trailingSpaces.length ? trailingSpaces : @"";
    NSString *trimmedString = [self jk_trimmedString];
    NSString *uncommentedCode =
        [trimmedString substringWithRange:NSMakeRange(2, trimmedString.length - 4)];
...
- (NSString *)jk_commentRemoved2String {
  NSString *trimmedString = [self jk_trimmedString];
  if ([trimmedString jk_isAcomment2]) {
    NSRegularExpression *expression =
        [[NSRegularExpression alloc] initWithPattern:@"^\\s*" options:0 error:NULL];
    NSRange range =
        [expression rangeOfFirstMatchInString:self options:0 range:NSMakeRange(0, self.length)];
    ...
    NSString *trailingSpaces = [self substringWithRange:range];
    trailingSpaces = trailingSpaces.length ? trailingSpaces : @"";
    trimmedString = [self jk_trimmedString];
    NSString *uncommentedCode =
        [trimmedString substringWithRange:NSMakeRange(2, trimmedString.length - 2)];
    return [NSString stringWithFormat:@"%@%@%@", leadingSpaces, uncommentedCode, trailingSpaces];
  }
  return self;
}
...
- (BOOL)jk_isAcomment2 {
  ...
  NSString *startString = [trimmedString substringToIndex:2];
  return [startString isEqualToString:@"//"];
}
